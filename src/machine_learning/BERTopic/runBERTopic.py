...
class BERTopicAnalysis:
    ...
    # initialize class
    def __init__(self, input_data, data_type, output_folder, output_db, k_cluster, do_inference, cmul_gpu):
    ...
    def load_data_telegram(self):
        ...
        i_database_name, i_collection_name = self.input_data.split('.')

        # load data from MongoDB
        cluster = self.initialize_database()
        data = cluster[i_database_name][i_collection_name]
        self.df = pd.DataFrame(list(data.find()))

        self.df = self.df.sample(frac=.2)  # TODO zhengyuan: just for testing. should remove in prd env
        print(len(self.df))
        ...
        lines = self.df[
            (self.df['messageText'].str.len() >= 100) & (self.df['messageText'].str.len() <= 2500)].messageText.values
    ...
    def fit_BERTopic(self):
        ...
        vectorizer_model = CountVectorizer(ngram_range=(1, 2),
                                           stop_words=stopWords)  # define vectorizer model with stopwords
    ...
    def save_model(self):
        ...
        self.model.save(f"{self.output_folder}/BERTopicmodel", serialization="safetensors", save_ctfidf=True,
                        save_embedding_model=embedding_model)

        # import gridfs
        # import pickle
        # cluster = self.initialize_database()
        # db = cluster[self.output_db]
        # db['fs.chunks'].drop()
        # db['fs.files'].drop()
        #
        # model = BERTopic.load(f"{self.output_folder}/BERTopicmodel")
        # pickled_model = pickle.dumps(model)
        #
        # fs = gridfs.GridFS(db)
        # docId = fs.put(pickled_model, filename='bertModel')  # put only accept

        # TODO zhengyuan: save model to remote env: mongodb or HuggingFace
    ...
    def write_representative_docs_df(self):
        ...
        for i in self.model.get_representative_docs().keys():
            ...
            self.model.get_topic_info()[self.model.get_topic_info()['Topic'] == i]['Name'].values[0][:31])
        writer.close()
...
def main():
    """
    use command to execute code
    python src/machine_learning/BERTopic/runBERTopic.py -i scrape.telegram -d telegram -of data/telegram/results/test/20230907/ -od bertModelTelegram -k auto

    """
    # # define parse arguments
    # parser = argparse.ArgumentParser()
    # parser.add_argument('-i', '--input_data', help="Specify the input file or folder", type=validate_database, required=True)
    # parser.add_argument('-d', '--data_type', choices=['telegram', 'twitter', 'google_news', 'gdelt'], help='Choose a datasource', required=True)
    # parser.add_argument('-of', '--output_folder', help="Specify folder for results", required=True)
    # parser.add_argument('-od', '--output_database', help="Specify database for results", required=True)
    ...
    #   reduce the size of model: use safetensor??
    #   store model to remote env
    ...
    if data_type == "twitter" or data_type == "google_news":
        with open("data/stopwords/stopwords_ua.txt") as file:  # load list of countries
            ...
            for country_stopword in country_stopwords:
                stopWords.append(country_stopword)
    BERTopic_Analysis = BERTopicAnalysis(input_data,
                                         data_type,
                                         output_folder,
                                         output_db,
                                         k_cluster,
